...
df = df.drop(df[df['secName'].str.contains('ST',regex=False)==True].index)#删除所有ST、*ST股票
df = df.drop(df[df['secName'].str.contains('退市',regex=False)==True].index)#删除所有退市股票
df = df.drop(df[df['secName'].str.contains('B',regex=False)==True].index)#删除所有900开头B股
df = df.drop(df[df['secName'].str.contains('Ｂ',regex=False)==True].index)#删除所有900开头B股
